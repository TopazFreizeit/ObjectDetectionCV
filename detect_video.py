...
def draw_detections(frame, detections, frame_w, frame_h, class_names):
    ...
    for i, (label, score) in enumerate(zip(labels, scores)):
        print(f"Detection {i}: Label Index = {label}, Score = {score:.2f}")
        if label > 0 and label < len(class_names):  # Skip background
            box = boxes[i]
            x1, y1, x2, y2 = box
            ...
            print(f"Skipping label {label}: Out of range or background.")

def process_video(video_path, output_path, model, config, class_names):
    ...
    while cap.isOpened():
        ...
        draw_detections(frame, detections, frame_width, frame_height, class_names)
        out.write(frame)
        frame_count += 1
        if frame_count % 10 == 0:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', default='output_video_2.mp4', type=str, help='Path to output video file')
    args = parser.parse_args()
    model, config = load_model(args.config, device)
    
    # Match the sorted order from dataset/voc.py
    class_names = ['background', 'aeroplane', 'bicycle', 'bird', 'boat', 'bottle', 'bus', 'car', 
                   'cat', 'chair', 'cow', 'diningtable', 'dog', 'horse', 'motorbike', 'person', 
                   'pottedplant', 'sheep', 'sofa', 'train', 'tvmonitor']
    
    print("Class names used for labeling:", class_names)
    print("Number of classes (including background):", len(class_names))
    print("Expected num_classes in config:", config['dataset_params']['num_classes'])
    
    process_video(args.video, args.output, model, config, class_names)